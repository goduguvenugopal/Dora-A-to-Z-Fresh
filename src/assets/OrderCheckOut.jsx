    ...
    html: `
      ...
      <div style="width: 100%; border: 1px solid gray;">
        ${orderProducts.map(item => `
          <div key="${item._id}" style="display: flex; gap: 12px; border-bottom: 2px solid gray; padding: 12px;">
            <img
...
<h3>Dora A to Z Fresh Address:</h3>
  ...
  useEffect(() => {
    ...
    const isOrderType = orderProducts?.some((item) => item.orderType === "subscription")
    ...
    if (defaultAddress.length <= 0) {
      ...
      try {
        ...
        if (res) {
          ...
          const emailRes = await axios.post(`${api}/updates-email/send-updates`, emailData);
          if (emailRes) {
            setOrderOk(true)
            setOrderSpin(false)
      ...
      <div className="mt-20 py-5 px-3 pb-10 ">
        ...
        {orderOk && <div className='px-5 fixed flex justify-center h-screen w-screen items-center top-0 left-0 bg-white text-black'>
          <div className='text-center flex flex-col items-center justify-center'>
            ...
            <Link to="/orders" className="text-white font-semibold bg-blue-600 mt-2 p-1 px-2 rounded">Go to My Orders</Link>
          </div>
